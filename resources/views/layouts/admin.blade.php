    ...
    <aside class="w-64 bg-indigo-900 text-indigo-100 flex flex-col p-6 space-y-8 sticky top-0 h-screen overflow-y-auto">
        ...
        <nav class="flex-1 space-y-2">
            ...
            @if(auth()->check() && auth()->user()->isSuperAdmin())
                <p class="text-[10px] font-bold uppercase tracking-widest text-indigo-400 mt-6 mb-4 px-2">Super Admin Area</p>
                ...
                <a href="{{ route('admin.organizers.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.organizers.*') ? 'bg-indigo-800 text-white' : 'hover:bg-indigo-800' }} rounded-xl font-bold transition">
                    <svg class="w-5 h-5 {{ request()->routeIs('admin.organizers.*') ? 'text-indigo-300' : 'text-indigo-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5m0 0h5m0 0v-5a2 2 0 012-2h2a2 2 0 012 2v5m-6 0v-5a2 2 0 00-2-2H9a2 2 0 00-2 2v5"></path>
                    </svg>
                    Kelola Organizer
                </a>
                ...
                <a href="{{ route('admin.promo-codes.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.promo-codes.*') ? 'bg-indigo-800 text-white' : 'hover:bg-indigo-800' }} rounded-xl font-bold transition">
                    <svg class="w-5 h-5 {{ request()->routeIs('admin.promo-codes.*') ? 'text-indigo-300' : 'text-indigo-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-center gap-4">
                <div class="text-right hidden md:block">
                    ...
                    <p class="text-xs text-indigo-600 font-bold uppercase tracking-wider">
                        {{ auth()->user()->isSuperAdmin() ? 'Super Admin' : (auth()->user()->organizer?->name ?? 'Organizer') }}
                    </p>
                </div>
                <div class="w-12 h-12 bg-white rounded-2xl shadow-sm border flex items-center justify-center p-1">
                    <img src="{{ auth()->user()->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name ?? 'Admin') . '&background=6366f1&color=fff' }}" class="rounded-xl w-full h-full object-cover">
                </div>
            </div>
        </header>
