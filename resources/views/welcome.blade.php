   ...
   <!-- Hero Section -->
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
            ...
            <div class="flex overflow-x-auto pb-2 gap-3 no-scrollbar scroll-smooth w-full md:w-auto">
                <a href="{{ route('home') }}#events" 
                    class="whitespace-nowrap px-6 py-2.5 rounded-xl font-bold transition-all duration-300 {{ !request('category') ? 'bg-slate-200 text-slate-700 shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                    Semua Kategori
                </a>
                @foreach($categories as $category)
                    <a href="{{ route('home', ['category' => $category->slug]) }}#events" 
                        class="whitespace-nowrap px-6 py-2.5 rounded-xl font-bold transition-all duration-300 {{ request('category') == $category->slug ? 'bg-indigo-100 text-indigo-700 shadow-sm' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100' }}">
                        {{ $category->name }}
                    </a>
                @endforeach
            </div>
    ...
    <!-- Partners Section -->
    <section class="border-t border-slate-100 py-20 bg-slate-50">
        <div class="max-w-7xl mx-auto px-6">
            <div class="text-center max-w-xl mx-auto mb-12">
                <h2 class="text-3xl font-extrabold mb-4">Partner & Sponsor Kami</h2>
                <p class="text-slate-500 font-medium">AmikomEventHub didukung oleh berbagai instansi dan perusahaan terpercaya.</p>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 items-center justify-items-center">
                @forelse($partners as $partner)
                    <div class="flex flex-col items-center justify-center p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition w-full h-32">
                        ...
                        @endif
                        <span class="text-xs font-bold text-slate-500 text-center">{{ $partner->name }}</span>
                    </div>
                @empty
                    <p class="col-span-full text-slate-400 text-center italic">Belum ada partner terdaftar.</p>
                @endforelse
            </div>
        </div>
    </section>
