...
@section('page_subtitle', 'Buat kupon diskon baru untuk pembeli tiket')

@section('content')
<div class="max-w-3xl mx-auto bg-white rounded-2xl p-8 border border-slate-100 shadow-sm">
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-xl font-bold">Form Tambah Kode Promo</h2>
    ...
    <form action="{{ route('admin.promo-codes.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="code" value="{{ old('code') }}" placeholder="Contoh: DISKON50" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 uppercase font-bold" required>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="discount_amount" value="{{ old('discount_amount') }}" placeholder="Contoh: 15000 atau 10" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 font-medium" required min="0" step="any">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="min_transaction" value="{{ old('min_transaction', 0) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 font-medium" min="0">
            </div>

            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Maksimal Diskon (Rp - Opsional)</label>
                <input type="number" name="max_discount" value="{{ old('max_discount') }}" placeholder="Khusus persentase" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 font-medium" min="0">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <input type="number" name="usage_limit" value="{{ old('usage_limit', 100) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 font-medium" required min="1">
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="expired_at" value="{{ old('expired_at') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 font-medium">
            </div>
        </div>
