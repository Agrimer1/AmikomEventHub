...
@section('page_subtitle', 'Daftar semua jabatan kepengurusan.')
...
@section('content')
    <div class="bg-white rounded-[2rem] shadow-sm border overflow-hidden">
        <div class="p-8 border-b flex justify-between items-center bg-white/50 backdrop-blur">
            <h3 class="text-xl font-bold">Daftar Jabatan</h3>
        ...
        <div class="px-8 py-4 border-b bg-slate-50/50 flex flex-wrap gap-4 items-center justify-between">
            <form action="{{ route('admin.jabatan.index') }}" method="GET" class="flex gap-2 w-full md:max-w-md">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.jabatan.index') }}" class="px-4 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-xl text-sm transition text-center">
                        Reset
                    </a>
                @endif
            </form>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 text-slate-500 uppercase text-xs font-bold tracking-widest">
                        ...
                        <th class="px-8 py-4">Diperbarui Oleh</th>
                ...
                <tbody class="divide-y">
                    @forelse($jabatans as $jabatan)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-8 py-6">
                                <span class="text-slate-600 font-medium">{{ $jabatan->id }}</span>
                            </td>
                            <td class="px-8 py-6">
                                <span class="font-bold text-lg text-slate-900">{{ $jabatan->name }}</span>
                            </td>
                            <td class="px-8 py-6">
                                <span class="text-slate-600">{{ $jabatan->created_by ?? '-' }}</span>
                            </td>
                            <td class="px-8 py-6">
                                <span class="text-slate-600">{{ $jabatan->updated_by ?? '-' }}</span>
                            </td>
                            <td class="px-8 py-6 text-slate-500 text-sm">
                                {{ $jabatan->created_at ? $jabatan->created_at->format('Y-m-d H:i') : '-' }}
                            </td>
                            <td class="px-8 py-6 text-slate-500 text-sm">
                                {{ $jabatan->updated_at ? $jabatan->updated_at->format('Y-m-d H:i') : '-' }}
                            </td>
                            <td class="px-8 py-6 text-slate-900 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.jabatan.destroy', $jabatan->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus jabatan ini? Semua pengurus dengan jabatan ini juga akan terhapus.')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-8 py-20 text-center text-slate-500">Belum ada data jabatan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
